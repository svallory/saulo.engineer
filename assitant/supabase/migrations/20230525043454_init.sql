...
create or replace function "docs"."match_page_sections"(embedding vector(768), match_threshold float, match_count int, min_content_length int)
...
begin
  ...
  select
    ...
    (page_section.embedding <=> embedding) as similarity
  from page_section

  -- We only care about sections that have a useful amount of content
  where length(page_section.content) >= min_content_length

  and (page_section.embedding <=> embedding) > match_threshold
